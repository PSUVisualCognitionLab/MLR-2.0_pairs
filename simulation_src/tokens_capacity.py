...
#from plots import test_dataset_MNIST_NC
...
def binding_cue(bs_testing, perms,bpsize, bpPortion, shape_coeff, color_coeff,samediff, modelNumber ):
    ...
    test_loader_smaller = torch.utils.data.DataLoader(dataset=test_filtered, batch_size=1, shuffle=True,
                                                      num_workers=0)
    ...
    for digit in range(0,10):
        ...
        test_loader[digit] = torch.utils.data.DataLoader(dataset=test_filtered, batch_size=1, shuffle=True,
                                                  num_workers=0)
...
################################cross correlation for L1 stright
    ...
    for rep in range(perms):
        ...
        if famnovel =="fam" :
            ...
            for img_novel in range(setSize):
                ...
                images.append(image)
                # save_image(images,'novelTrans.png')
        ...
        if layernum==1:#staright recon from L1 using skip
            l1_act_tr = l1_act.clone()
            if memory==0:
                retrievals, mu_color, log_var_color, mu_shape, log_var_shape = vae.forward_layers(l1_act_tr, l2_act, 1,
                                                                                                  'skip')
                ...
                # l1_out_all = torch.pow(10, l1_out_all) - 1
                l1_out_all[l1_out_all < 0] = 0

                retrievals, mu_color, log_var_color, mu_shape, log_var_shape = vae.forward_layers(l1_out_all, l2_act,
                                                                                                  1,
                                                                                                  'skip')
